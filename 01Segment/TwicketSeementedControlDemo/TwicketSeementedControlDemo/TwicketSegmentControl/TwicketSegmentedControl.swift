...
public protocol TwicketSegmentedControlDelegate: class  {
    ...
    /// 选中某个 segment item 调用此方法
    ///
    /// - Parameter segmentIndex: 选中的segment item 的索引
...
open class TwicketSegmentedControl: UIControl {
    ...
    private struct Constants {
        ...
        static let topBottomMargin: CGFloat = 5 // 上下边距
        static let leadingTrailingMargin: CGFloat = 10 // 前后边距
    }
    
    public static let height: CGFloat = Constants.height + Constants.topBottomMargin * 2 // height = 30 + 5 * 2 = 40
    ...
    private func configureViews() {
        ...
        let conerRadius = backgroundView.frame.height / 2
        [backgroundView, selectedContainerView].forEach { (aView) in
            aView.layer.cornerRadius = conerRadius
        }
        sliderView.cornerRadius = conerRadius
    ...
    private func clearLabels() {
        ...
        selectedContainerView.subviews.forEach { (aView) in
    ...
    private func updateLabelsColor(with color: UIColor, selected: Bool) {
        ...
        containerView.subviews.forEach { (aView ) in
            (aView as? UILabel)?.textColor = color
        }
    }
    
    private func updateLabelsFont(with font: UIFont) {
        selectedContainerView.subviews.forEach { (aView) in
            (aView as? UILabel)?.font = font
        }
        
        backgroundView.subviews.forEach { (aView) in
            (aView as? UILabel)?.font = font
        }
    }
    ...
    /// 根据传入的 index ，返回偏移量
    ///
    /// - Parameter index: segment 的 index 值
    /// - Returns: 偏移量
    ...
    /// 动画移动 sliderView
    ///
    /// - Parameter postion: 位置
    private func animate(to postion: CGFloat) {
        UIView.animate(withDuration: 0.2) {
            self.sliderView.center.x  = postion
        }
    }
}
